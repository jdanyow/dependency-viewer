...
import d3 from 'fzaninotto/DependencyWheel/js/d3.min';
import dw from 'fzaninotto/DependencyWheel/js/d3.dependencyWheel';
...
export class App {
  ...
  constructor(observerLocator) {
    ...
    this.readConfig();
    observerLocator.getObserver(this, 'config')
      .subscribe(this.configChanged.bind(this));
  }
  ...
  render() {
    var chart = d3.chart.dependencyWheel()
      .width(window.innerHeight)    // also used for height, since the wheel is in a a square
...
function buildPackageNames(map, data) {
  var names = Object.keys(map), name, i;
...
function buildDepArray(map, data) {
  var deps = [0], key, index = {};
  Object.keys(map).forEach(key => index[map[key]] = true);
...
function buildDataFromSystemJSMap(map) {
  ...
  data.packageNames = data.packageNames.map(name => {
    var i = name.indexOf(':');
    if (i === -1) {
      return name;
    }
  ...
  data.packageNames.slice(1).sort((a, b) => a.toLowerCase().localeCompare(b.toLowerCase())).splice(0, 0, 'root');
  return data;
}


var testData = `System.config({
  "paths": {
    "*": "*.js",
